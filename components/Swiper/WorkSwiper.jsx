...
export const WorkSwiper = ({ swiperRef }) => {
    ...
    return (
        <Swiper onSwiper={(swiper) => { swiperRef.current = swiper }} slidesPerView={1} spaceBetween={10} className='w-full h-full' >
            {works.map((work, index) => (
                <SwiperSlide key={index} className='relative sm:!mr-0 flex'  >
                    <div className='flex flex-col-reverse sm:flex-row mt-10 min-h-[32rem] h-full rounded-3xl bg-[#D9D9D9] justify-between' >
                        ...
                        { work.video && <div className='work_card flex-center overflow-hidden' >
